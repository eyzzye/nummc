...
static int inventory_manager_register_weapon(unit_items_data_t* item_data, int list_index)
{
	g_stocker.weapon_item->item_count += 1;
	return 0;
}
...
int inventory_manager_register_item(unit_items_data_t* item_data)
{
	...
	if (list_index >= 0) {
		inventory_manager_register_weapon(item_data, list_index);
		g_stocker.version += 1;
		return 0;
	}
	...
	if (list_index >= 0) {
		inventory_manager_register_charge(item_data, list_index);
		g_stocker.version += 1;
		return 0;
	}
	...
	if (list_index >= 0) {
		inventory_manager_register_special(item_data, list_index);
		g_stocker.version += 1;
		return 0;
	}
...
int inventory_manager_set_weapon_count(int count)
{
	int item_list_idx = g_stocker.weapon_item->item_list_idx;
	if (item_list_idx >= 0) { // registed item
		int val = g_stocker.weapon_item->item_count;
