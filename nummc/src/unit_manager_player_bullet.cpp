...
static node_buffer_info_t player_bullet_base_info;
static node_buffer_info_t player_bullet_info;
...
int unit_manager_init_player_bullet()
{
	...
	game_utils_node_init(&player_bullet_base_info, sizeof(unit_player_bullet_data_t));
	for (int i = 0; i < UNIT_PLAYER_BULLET_BASE_LIST_SIZE; i++) {
		player_bullet_base[i] = (unit_player_bullet_data_t*)game_utils_node_new(&player_bullet_base_info);
	}

	game_utils_node_init(&player_bullet_info, sizeof(unit_player_bullet_data_t));
	for (int i = 0; i < UNIT_PLAYER_BULLET_LIST_SIZE; i++) {
		player_bullet[i] = (unit_player_bullet_data_t*)game_utils_node_new(&player_bullet_info);
	}
...
void unit_manager_unload_player_bullet()
{
	for (int i = 0; i < UNIT_PLAYER_BULLET_BASE_LIST_SIZE; i++) {
		...
		game_utils_node_delete((node_data_t*)player_bullet_base[i], &player_bullet_base_info);
		player_bullet_base[i] = NULL;
	}

	for (int i = 0; i < UNIT_PLAYER_BULLET_LIST_SIZE; i++) {
		if (player_bullet[i]->obj) {
			memory_manager_delete_char_buff((char*)player_bullet[i]->obj);
			player_bullet[i]->obj = NULL;
		}
		game_utils_node_delete((node_data_t*)player_bullet[i], &player_bullet_info);
		player_bullet[i] = NULL;
	}
}
...
int unit_manager_load_player_bullet(char* path)
{
	...
	if (tmp_path_size == 0) { LOG_ERROR("unit_manager_load_player_bullet failed get %s\n", path); return 1; }
	...
	if (ret != 0) { LOG_ERROR("unit_manager_load_player_bullet %s error\n", path); return 1; }
