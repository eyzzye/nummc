...
static node_buffer_info_t enemy_bullet_base_info;
static node_buffer_info_t enemy_bullet_info;
...
int unit_manager_init_enemy_bullet()
{
	...
	game_utils_node_init(&enemy_bullet_base_info, sizeof(unit_enemy_bullet_data_t));
	for (int i = 0; i < UNIT_ENEMY_BULLET_BASE_LIST_SIZE; i++) {
		enemy_bullet_base[i] = (unit_enemy_bullet_data_t*)game_utils_node_new(&enemy_bullet_base_info);
	}

	game_utils_node_init(&enemy_bullet_info, sizeof(unit_enemy_bullet_data_t));
	for (int i = 0; i < UNIT_ENEMY_BULLET_LIST_SIZE; i++) {
		enemy_bullet[i] = (unit_enemy_bullet_data_t*)game_utils_node_new(&enemy_bullet_info);
	}
...
void unit_manager_unload_enemy_bullet()
{
	for (int i = 0; i < UNIT_ENEMY_BULLET_BASE_LIST_SIZE; i++) {
		...
		game_utils_node_delete((node_data_t*)enemy_bullet_base[i], &enemy_bullet_base_info);
		enemy_bullet_base[i] = NULL;
	}

	for (int i = 0; i < UNIT_ENEMY_BULLET_LIST_SIZE; i++) {
		if (enemy_bullet[i]->obj) {
			memory_manager_delete_char_buff((char*)enemy_bullet[i]->obj);
			enemy_bullet[i]->obj = NULL;
		}
		game_utils_node_delete((node_data_t*)enemy_bullet[i], &enemy_bullet_info);
		enemy_bullet[i] = NULL;
	}
}
